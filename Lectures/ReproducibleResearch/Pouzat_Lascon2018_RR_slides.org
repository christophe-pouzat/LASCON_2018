...
* Notes that are archived but can evolve with concurrent version systems (CVS)
...
\vspace{0.25cm}
#+END_EXPORT
Clicking /previous versions/ (/anciennes révisions/) gives access to the list of changes done when and by whom. If I know select two versions...
